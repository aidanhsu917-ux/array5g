import adi
import numpy as np
import pyqtgraph as pg # pyqtgraph will plot MUCH faster than matplotlib
(https://pyqtgraph.readthedocs.io/en/latest/getting_started/installation.html)
from pyqtgraph.Qt import QtCore, QtGui
from half_moon_compass import HalfMoonCompass
compass = HalfMoonCompass()
'''Setup'''
...
d_wavelength = 0.5 # distance between elements as a fraction of wavelength. This is
normally 0.5
...
def monopulse_angle(array1, array2):
 ...
 # Since our signals are closely aligned in time, we can just return the 'valid' case where the
signals completley overlap
...
def scan_for_DOA():
 # go through all the possible phase shifts and find the peak, that will be the DOA (direction of
arrival) aka steer_angle
 data = sdr.rx()
 Rx_0=data[0]
 Rx_1=data[1]
 peak_sum = []
...
return delay_phases, peak_dbfs, peak_delay, steer_angle, peak_sum, peak_delta,
monopulse_phase
...
def Tracking(last_delay):
 ...
 delayed_delta_fft, delayed_delta_dbfs = dbfs(delayed_delta)
 mono_angle = monopulse_angle(delayed_sum_fft, delayed_delta_fft)
 angle = calcTheta(delay) # whatever angle you already compute
 compass.update(angle)
 phase_step= 1
...
for i in range(20):
 ...
 #scan once to get the direction of arrival (steer_angle) as the initial point for out monopulse
tracker
delay_phases, peak_dbfs, peak_delay, steer_angle, peak_sum, peak_delta, monopulse_phase =
scan_for_DOA()
...
if __name__ == '__main__':
 import sys
 iapp = pg.mkQApp() # works across PyQt5/PyQt6/PySide
